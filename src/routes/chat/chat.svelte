<script lang="ts">
    ...
    onMount(async () => {
        fetch('/chat/models/list')   
            .then(res => res.json())
            .then(data => {
                try {
                    Object.entries(data.models).forEach(([key, value]) => {
                        availableModels.push({id: key, name: value as string})
                    })
                }catch (error) {
                    goto('/')
                }
    ...
    async function queryModel(model: string, message: string) {
        const response = await fetch('/chat/models/query', {
            method: 'POST',
            headers: {
    ...
    async function sendMessage() {
        if (inputValue.trim()) {
            ...
            try {
                const response = await queryModel(selectedModel, userMessage)
                if (response.response == undefined) {
                    messages.push({content: "Error while generating", author: 'AI', model: "error while generating"});
                }else{
                    messages.push({content: response.response.content, author: 'AI', model: response.response.model});
                }
            }catch (error) {
                goto('/')
            }
